...
```{r}
path <- "/home/brymoore/wsg_gtdb_tax/pig_microbiome/PRJNA1387191/fastq_files" #Change me to the directory containing the fastq files(unzipped)
...
```
**These are some really bad error rates**
...
```
*there is a large drop in my merged sequences, have to revisit filter and trim, when went to retrim, decided not to select a trunk length and it is much better with not large drop off at merging
```{r}
